...
public class CakeCompassIndicator : MonoBehaviour
{
	...
	{
		Vector3 position = m_camera.transform.position;
		if (Mathf.Abs(m_cake.transform.position.y - position.y) < m_camera.orthographicSize)
		{
			return Mathf.Abs(m_cake.transform.position.x - position.x) < m_camera.orthographicSize * (float)Screen.width / (float)Screen.height;
		}
		return false;
	}
}
