...
public class BirdCompassIndicator : WPFMonoBehaviour
{
	...
	private void UpdateArrow()
	{
		if (!(m_bird == null))
		{
			...
			Vector2 vector = m_bird.transform.position - m_camera.transform.position;
			vector.Normalize();
	...
	public bool CheckBirdOnScreen()
	{
		...
		if (Mathf.Abs(m_bird.transform.position.y - position.y) < m_camera.orthographicSize)
		{
			return Mathf.Abs(m_bird.transform.position.x - position.x) < m_camera.orthographicSize * (float)Screen.width / (float)Screen.height;
		}
		return false;
	}
}
