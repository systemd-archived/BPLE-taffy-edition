...
public class ElectricalSystem : PartManager
{
	...
	private void UpdatePowerTransmitters()
	{
		...
		foreach (PowerTransmitterPart item in list2)
		{
			...
			float num = 1024f;
			float num2 = item.GetChannel();
			PowerTransmitterPart powerTransmitterPart2 = null;
			foreach (PowerTransmitterPart item2 in list)
			{
				Vector3 position2 = item2.transform.position;
				float num3 = Vector.DistanceSquared2(position, position2);
				if ((float)item2.GetChannel() == num2 && num3 < num)
				{
					num = num3;
					powerTransmitterPart2 = item2;
				}
			}
			if (powerTransmitterPart2 != null)
			{
				item.Connect(powerTransmitterPart2, Mathf.Sqrt(num));
			}
			else
			{
				item.Disconnect();
